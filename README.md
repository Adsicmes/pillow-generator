...
- **实时画布编辑**：所有修改都会实时反映到编辑画布
- **Pillow 结果预览**：可直接预览按 Pillow 语义渲染后的最终图像
...
### 6. 预览结果
- 点击 `代码 -> 预览结果` 或工具栏中的“预览”按钮
- 应用会使用当前项目配置直接渲染 Pillow 图像并显示结果
- 预览窗口显示的是最终 Pillow 渲染结果；中间画布主要用于编辑布局，显示可能略有近似
...
### 8. 保存生成代码
- 点击代码生成器中的“保存代码”按钮
- 保存时会按目标 `.py` 文件位置生成资源相对路径，便于脚本单独移动和分发
...
### 数据管理
- **JSON格式**：人类可读的项目文件格式
- **相对路径**：保存项目与导出代码时都会尽量按目标文件位置生成相对路径
...
- 撤销 / 重做：需要围绕项目模型建立统一的命令历史，而不是零散地在各个控件中回滚状态
- 图层锁定：需要同时约束画布拖动、属性编辑和图层面板操作，适合作为单独一轮交互改造
